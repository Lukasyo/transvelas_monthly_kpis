...
#nurodom, kurios masinos yra kubai
kubai <- c("FFG506",
          "FFG508",
          "GHS848",
          "GHS878",
...
#nurodom, kurios mašinos nevažiavo pilnų metų
...
#pridedam likusius klientus
rest <- all[!all$reisas %in% data$reisas,]
rest$klientas <- "Kiti"
...
#atskiriam kurie kubai, kurie paprasti
data_full <- data
data_full[data_full$masina %in% kubai,"tipas"] <- "kubas"
data_full[is.na(data_full$tipas),"tipas"] <- "paprasta"

# paliekam tik kubus
data <- data[data$masina %in% kubai,]
...
# Mašinų tipų rodiklių skaičiavimas -------------------------------------------

#skaičiuojam frachtus dienai paketvirčiui kiekvienam klientui
data_full$frachtas <- as.numeric(data_full$frachtas)
data_full$is_viso_dienu <- as.integer(data_full$is_viso_dienu)
data_full$frachtas_dienai <- data_full$frachtas/data_full$is_viso_dienu
frachtai_tipas_ketvirtis <- aggregate(frachtas_dienai ~ tipas + ketvirtis, data = data_full, FUN = sum)

#skaičiuojam pelną dienai paketvirčiui kiekvienam klientui
data_full$pelnas_pries_pap_islaidas <- as.numeric(data_full$pelnas_pries_pap_islaidas)
pelnas_tipas_ketvirtis <- aggregate(pelnas_pries_pap_islaidas ~ tipas + ketvirtis, 
                                       data = data_full, FUN = sum)
reisai_tipas <- aggregate(is_viso_dienu ~ tipas + ketvirtis, data = data_full, FUN = sum)
pelnas_tipas_ketvirtis <- left_join(pelnas_tipas_ketvirtis,reisai_tipas)
pelnas_tipas_ketvirtis$pelnas_dienai2 <- pelnas_tipas_ketvirtis$pelnas_pries_pap_islaidas / 
  pelnas_tipas_ketvirtis$is_viso_dienu
pelnas_proc_tipas_ketvirtis <- left_join(frachtai_tipas_ketvirtis,pelnas_tipas_ketvirtis)
pelnas_proc_tipas_ketvirtis$pelnas_proc <- pelnas_proc_tipas_ketvirtis$pelnas_dienai2 / 
  pelnas_proc_tipas_ketvirtis$frachtas_dienai * 100

# skaičiuojam metinį vidutinį pelną dienai kiekvienam klientui
pelnas_tipas_metai <- aggregate(pelnas_pries_pap_islaidas  ~ tipas, data = data_full, FUN = sum)
reisai_tipas <- aggregate(is_viso_dienu ~ tipas, data = data_full, FUN = sum)
pelnas_tipas_metai <- left_join(pelnas_tipas_metai,reisai_tipas)
pelnas_tipas_metai$pelnas_dienai2 <- pelnas_tipas_metai$pelnas_pries_pap_islaidas /
  pelnas_tipas_metai$is_viso_dienu

#grafikas
pelnas_plot <- pelnas_tipas_ketvirtis[pelnas_tipas_ketvirtis$ketvirtis == ketvirtis,]
pelnas_plot <- pelnas_plot[,!colnames(pelnas_plot)=="ketvirtis"]
pelnas_plot$periodas <- paste(ketvirtis,"ketvirtis")
pelnas_tipas_metai$periodas <- paste(metai,"metai")
pelnas_plot <- rbind(pelnas_plot,pelnas_tipas_metai)

ggplot(data = pelnas_plot, aes(x = reorder(tipas, -pelnas_dienai2), y = pelnas_dienai2)) +
  geom_col(fill = "steelblue") +
  scale_y_continuous(breaks = round(seq(0, max(pelnas_plot$pelnas_dienai2), by = 10),0)) +
  ylab("EUR") + 
  ggtitle("Pelnas dienai (EUR) pagal mašinos tipą") +
  xlab("Tipas") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1,size=13),
        axis.text.y = element_text(angle = 0, hjust = 1,size=13),
        axis.title.y = element_text(size = rel(1.4), angle = 90),
        axis.title.x = element_text(size = rel(1.4), angle = 0),
        plot.title = element_text(size = 24)) +
  geom_text(aes(label = round(pelnas_dienai2,1)), size = 5) + 
  facet_grid(cols = vars(periodas))

# kubų pelnas per metus

data_full <- data_full[!data_full$masina %in% ner_metu,]

# sumuojam mašinų pelną per metus
data_full$pelnas_pries_pap_islaidas <- as.numeric(data_full$pelnas_pries_pap_islaidas)
pelnas_masina_metai <- aggregate(pelnas_pries_pap_islaidas ~ masina, 
                                data = data_full, FUN = sum)
pelnas_masina_metai <- left_join(pelnas_masina_metai,data_full[,c("masina","tipas")], 
                                 match = "first")
pelnas_masina_metai <- pelnas_masina_metai[!duplicated(pelnas_masina_metai$masina),]

#skaičiuojam pelną per metus kiekvienam tipui
pelnas_tipas_metai <- aggregate(pelnas_pries_pap_islaidas ~ tipas, 
                                    data = pelnas_masina_metai, FUN = mean)


#grafikas
pelnas_plot <- pelnas_tipas_metai

ggplot(data = pelnas_plot, aes(x = reorder(tipas, -pelnas_pries_pap_islaidas), y = pelnas_pries_pap_islaidas)) +
  geom_col(fill = "steelblue") +
  scale_y_continuous(breaks = round(seq(0, max(pelnas_plot$pelnas_pries_pap_islaidas), by = 5000),0)) +
  ylab("EUR") + 
  ggtitle("Pelnas (EUR) pagal mašinos tipą per metus") +
  xlab("Tipas") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1,size=13),
        axis.text.y = element_text(angle = 0, hjust = 1,size=13),
        axis.title.y = element_text(size = rel(1.4), angle = 90),
        axis.title.x = element_text(size = rel(1.4), angle = 0),
        plot.title = element_text(size = 24)) +
  geom_text(aes(label = round(pelnas_pries_pap_islaidas,0)), size = 5)


# Mašinų rodiklių skaičiavimas -------------------------------------------

#skaičiuojam frachtus dienai paketvirčiui kiekvienam klientui
data$frachtas <- as.numeric(data$frachtas)
data$is_viso_dienu <- as.integer(data$is_viso_dienu)
data$frachtas_dienai <- data$frachtas/data$is_viso_dienu
frachtai_masina_ketvirtis <- aggregate(frachtas_dienai ~ masina + ketvirtis, data = data, FUN = sum)

#skaičiuojam pelną dienai paketvirčiui kiekvienai masinai
pelnas_masina_ketvirtis <- aggregate(pelnas_pries_pap_islaidas ~ masina + ketvirtis, 
                                       data = data, FUN = sum)
reisai_masina <- aggregate(is_viso_dienu ~ masina + ketvirtis, data = data, FUN = sum)
pelnas_masina_ketvirtis <- left_join(pelnas_masina_ketvirtis,reisai_masina)
pelnas_masina_ketvirtis$pelnas_dienai2 <- pelnas_masina_ketvirtis$pelnas_pries_pap_islaidas / 
  pelnas_masina_ketvirtis$is_viso_dienu

# skaičiuojam metinį vidutinį pelną dienai kiekvienam klientui
pelnas_masina_metai <- aggregate(pelnas_pries_pap_islaidas  ~ masina, data = data, FUN = sum)
reisai_masina <- aggregate(is_viso_dienu ~ masina, data = data, FUN = sum)
pelnas_masina_metai <- left_join(pelnas_masina_metai,reisai_masina)
pelnas_masina_metai$pelnas_dienai2 <- pelnas_masina_metai$pelnas_pries_pap_islaidas /
  pelnas_masina_metai$is_viso_dienu

# Frachtas pagal mašiną ir klientą skaičiavimas ir grafikas -------------------------------------------

frachtai_masina_klientas_ketvirtis <- aggregate(frachtas ~ klientas + masina + ketvirtis, data = data, FUN = sum)

frachtas_plot <- frachtai_masina_klientas_ketvirtis[frachtai_masina_klientas_ketvirtis$ketvirtis == ketvirtis,]

ggplot(data = frachtai_plot, aes(x = reorder(masina, -frachtas), y = frachtas,
                               group = klientas)) +
  geom_col(aes(fill = klientas)) +
  scale_y_continuous(breaks = round(seq(0, max(frachtas_plot$frachtas), by = 5000),0)) +
  ylab("EUR") + 
  ggtitle("Kubų frachtai 4 ketvirtį (EUR)") +
  xlab("Mašina") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1,size=13),
        axis.text.y = element_text(angle = 0, hjust = 1,size=13),
        axis.title.y = element_text(size = rel(1.4), angle = 90),
...
# Mašinų + klientų rodiklių skaičiavimas -------------------------------------------

#skaičiuojam frachtus dienai paketvirčiui kiekvienam klientui
frachtai_masina_klientas_ketvirtis <- aggregate(frachtas_dienai ~ masina + ketvirtis + klientas, data = data, FUN = sum)

#skaičiuojam pelną dienai paketvirčiui kiekvienai masinai
pelnas_masina_klientas_ketvirtis <- aggregate(pelnas_pries_pap_islaidas ~ masina + ketvirtis + klientas, 
                                     data = data, FUN = sum)
reisai_masina_klientas <- aggregate(is_viso_dienu ~ masina + ketvirtis + klientas, data = data, FUN = sum)
pelnas_masina_klientas_ketvirtis <- left_join(pelnas_masina_klientas_ketvirtis,reisai_masina_klientas)
pelnas_masina_klientas_ketvirtis$pelnas_dienai2 <- pelnas_masina_klientas_ketvirtis$pelnas_pries_pap_islaidas / 
  pelnas_masina_klientas_ketvirtis$is_viso_dienu

# skaičiuojam metinį vidutinį pelną dienai kiekvienam klientui
pelnas_masina_klientas_metai <- aggregate(pelnas_pries_pap_islaidas  ~ masina + klientas, data = data, FUN = sum)
reisai_masina_klientas <- aggregate(is_viso_dienu ~ masina + klientas, data = data, FUN = sum)
pelnas_masina_klientas_metai <- left_join(pelnas_masina_klientas_metai,reisai_masina_klientas)
pelnas_masina_klientas_metai$pelnas_dienai2 <- pelnas_masina_klientas_metai$pelnas_pries_pap_islaidas /
  pelnas_masina_klientas_metai$is_viso_dienu
...
ggplot(data = pelnas_plot, aes(x = reorder(klientas, -pelnas_dienai2), y = pelnas_dienai2)) +
  ...
  scale_y_continuous(breaks = round(seq(0, max(pelnas_plot$pelnas_dienai2), by = 10),0)) +
  ylab("EUR") + 
  ggtitle("Kubų pelnas dienai (EUR) 4 ketvirtis 2018") +
  xlab("Klientas") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1,size=13),
        axis.text.y = element_text(angle = 0, hjust = 1,size=13),
        axis.title.y = element_text(size = rel(1.4), angle = 90),
...
ggplot(data = pelnas_plot, aes(x = reorder(klientas, -pelnas_dienai2), y = pelnas_dienai2)) +
  ...
  scale_y_continuous(breaks = round(seq(0, max(pelnas_plot$pelnas_dienai2), by = 10),0)) +
  ylab("EUR") + 
  ggtitle("Kubų pelnas dienai (EUR) 2018 metai") +
  xlab("Klientas") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1,size=13),
        axis.text.y = element_text(angle = 0, hjust = 1,size=13),
        axis.title.y = element_text(size = rel(1.4), angle = 90),
...
# Pelnas dienai EUR pagal mašinas, 4 ketvirtis vs metai --------------------

#paskutinio ketvirčio pelno grafikas
pelnas_plot_ketv <- pelnas_masina_ketvirtis[pelnas_masina_ketvirtis$ketvirtis == ketvirtis,]
pelnas_plot_ketv$periodas <- "2018, IV ketvirtis"
pelnas_plot_ketv <- pelnas_plot_ketv[,!colnames(pelnas_plot_ketv) == "ketvirtis"]

#metinis pelno grafikas
pelnas_plot <- pelnas_masina_metai
pelnas_plot$periodas <- "2018 metai"
pelnas_plot <- rbind(pelnas_plot,pelnas_plot_ketv)
ggplot(data = pelnas_plot, aes(x = reorder(masina, -pelnas_dienai2), y = pelnas_dienai2,
                               group = periodas)) +
  geom_col(aes(fill = periodas) , position = position_dodge()) +
  scale_y_continuous(breaks = round(seq(0, max(pelnas_plot$pelnas_dienai2), by = 10),0)) +
  ylab("EUR") + 
  ggtitle("Kubų pelnas dienai (EUR)") +
  xlab("Mašina") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1,size=13),
        axis.text.y = element_text(angle = 0, hjust = 1,size=13),
        axis.title.y = element_text(size = rel(1.4), angle = 90),
...
# Pelnas dienai EUR pagal mašinas + klientą, 4 ketvirtis vs metai --------------------

#paskutinio ketvirčio pelno grafikas
pelnas_plot_ketv <- pelnas_masina_klientas_ketvirtis[pelnas_masina_klientas_ketvirtis$ketvirtis == ketvirtis,]
pelnas_plot_ketv$periodas <- "2018, IV ketvirtis"
pelnas_plot_ketv <- pelnas_plot_ketv[,!colnames(pelnas_plot_ketv) == "ketvirtis"]

#metinis pelno grafikas
pelnas_plot <- pelnas_masina_klientas_metai
pelnas_plot$periodas <- "2018 metai"
pelnas_plot <- rbind(pelnas_plot,pelnas_plot_ketv)

#pasirinkti klientą
pelnas_plot <- pelnas_plot[pelnas_plot$klientas == "Dedal",]
ggplot(data = pelnas_plot, aes(x = reorder(masina, -pelnas_dienai2), y = pelnas_dienai2,
                               group = periodas)) +
  geom_col(aes(fill = periodas) , position = position_dodge()) +
  scale_y_continuous(breaks = round(seq(0, max(pelnas_plot$pelnas_dienai2), by = 10),0)) +
  ylab("EUR") + 
  ggtitle("Dedal kubų pelnas dienai (EUR)") +
  xlab("Mašina") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1,size=13),
        axis.text.y = element_text(angle = 0, hjust = 1,size=13),
        axis.title.y = element_text(size = rel(1.4), angle = 90),
